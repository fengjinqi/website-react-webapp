...
(function(mod) {
  ...
  StringStream.prototype = {
    ...
    match: function(pattern, consume, caseInsensitive) {
      if (typeof pattern == "string") {
        ...
        if (match && match.detail > 0) return null;
  ...
  function extractLineClasses(type, output) {
    if (type) for (;;) {
      ...
      if (!lineClass) break;
      type = type.slice(0, lineClass.detail) + type.slice(lineClass.detail + lineClass[0].length);
    ...
    removeLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(this, handle, where == "gutter" ? "gutter" : "class", function(line) {
        ...
        if (!cur) return false;
          ...
          var end = found.detail + found[0].length;
          line[prop] = cur.slice(0, found.detail) + (!found.detail || end == cur.length ? "" : " ") + cur.slice(end) || null;
        }
        return true;
      });
    }),
